...
const GroupNoticeDetail = () => {
    ...
    return (
        <div>
            ...
            { (data.userPk === loginUser.userPk)? <ButtonStyle><button className="custom-btn btn-4" onClick={ModifyHandler}>Modify</button></ButtonStyle> : null } {/* 로그인 userPk와 글쓴 사람의 userPk가 같을 경우 수정 버튼 보이게 */}
            { (data.userPk === loginUser.userPk)? <ButtonStyle><button className="custom-btn btn-4" onClick={deleteHandler} style={ { float: 'left' }}>Delete</button></ButtonStyle> : null }
